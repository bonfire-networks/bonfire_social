if Application.compile_env(:bonfire_api_graphql, :modularity) != :disabled and
  ...
  defmodule Bonfire.Social.API.GraphQL do
    ...
    # Search for activities/statuses using meilisearch
    ...
    # Search for users/accounts using meilisearch
    ...
    # Shared helper for meilisearch-based queries
    # Searches meilisearch for IDs, then loads each entity via the provided loader function
    defp search_meilisearch(filter, info, index_type, loader_fn) do
      current_user = GraphQL.current_user(info)
      query = filter[:query] || ""

      # Note: Empty query check is in adapter layer, but kept for direct GraphQL calls
      if query == "" do
        {:ok, []}
      else
