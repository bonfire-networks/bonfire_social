defmodule Bonfire.Social.Seen do
  ...
  def seen?(%{}=user, object), do: not is_nil(get!(user, object, skip_boundary_check: true))
  ...
  def mark_seen(%User{} = subject, object) when is_binary(object) do
    with {:ok, seen} <- Bonfire.Common.Pointers.get(object, current_user: subject, verb: :see) do
      #debug(seen)
  ...
  def mark_unseen(%User{}=subject, %{}=object) do
    Edges.delete_by_both(subject, Seen, object) # delete the Seen
  ...
  def mark_unseen(%User{} = subject, object) when is_binary(object) do
    with {:ok, seen} <- Bonfire.Common.Pointers.get(object, current_user: subject) do
