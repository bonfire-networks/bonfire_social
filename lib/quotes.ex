defmodule Bonfire.Social.Quotes do
  ...
  def check_quote_permission(user, quoted_object, _opts \\ []) do
    ...
    cond do
      e(quoted_object, :created, :creator_id, nil) == user_id ||
        ...
        info(user_id, "User is quoting their own post")
  ...
  def accept_quote(quote_object, quoted_object, opts \\ []) do
    ...
    |> info("got quote request to accept on #{repo()}")
  ...
  defp federate_reject(:delete, request, quote_object, quoted_object) do
    with {:ok, ap_quote_object} <-
      ...
      {:error, :not_found} ->
        info("No quoteAuthorization found, nothing to delete")
        {:ok, :ignore}
  ...
  def update_quote_add(subject, quote_post, quoted_object, _opts) do
    # info(subject, "update_quote_add subject (requester)")
    # info(quote_post, "update_quote_add quote_post (post with attached quote)")
    # info(quoted_object, "update_quote_add object (quoted post)")
  ...
  def update_quote_remove(quote_post, quoted_object) do
    info(quote_post, "thing to remove tags from")
    info(quoted_object, "tags to remove from thing")
  ...
  def verify_quote_authorization(quote_post, quoted_object \\ nil, authorization \\ nil) do
    quoted_object =
      ...
      |> info("quoted_object for verification")
  ...
  def ap_publish_activity(
    ...
    info(request, "Publishing QuoteRequest activity")
    ...
    with {:ok, actor} <-
         ...
         {:ok, ap_quoted_object} <-
           ActivityPub.Object.get_cached(pointer: quoted_object)
           |> info("quoted_object for #{inspect(quoted_object)}"),
         {:ok, instrument} <-
           ActivityPub.Object.get_cached(pointer: id(quote_post))
           |> info("quote post for #{inspect(quote_post)}"),
         {:ok, activity} <-
           ...
           |> info("created AP quote request") do
      {:ok, activity}
    else
      {:error, :not_found} ->
        info("Actor, Object, or Quote Post not found", error_msg)
        {:ok, :ignore}
  ...
  def ap_receive_activity(
    ...
    info(data, "Received QuoteRequest activity")
    # info(quoted_object, "quoted_object")
    ...
    with {:ok, requester} <-
      ...
      case check_quote_permission(requester, quoted_object)
        ...
        {:auto_approve, quoted_object} ->
          with {:ok, quote_post} <-
               ...
               {:ok, _updated} <-
                 ...
                 |> info("Auto-accepted quote request and updated quote post") do
            {:ok, request}
          else
            e ->
        ...
        {:request_needed, quoted_object} ->
          with {:ok, quote_post} <-
                 Bonfire.Federate.ActivityPub.AdapterUtils.return_pointable(data["instrument"])
                 |> info("prepared quote post") do
            # Save request in our system for manual approval
            info("WIP: send notification of request to user")
        ...
        {:not_permitted, _quoted_object} ->
          ...
          |> info("Rejected quote request")
      end
    else
      {:error, e} ->
        info(e, "QuoteRequest error")
  ...
  def ap_receive_activity(
    ...
    info(quote_request_object, "Received Reject for QuoteRequest")
    ...
    with {:ok, local_quote_post} <-
         ...
         {:ok, updated_quote_post} <-
           ...
           |> info("Updated local quote post with rejection") do
      {:ok, updated_quote_post}
    else
      {:error, :not_found} ->
  ...
  def ap_receive_activity(
    ...
    info(object, "Received Delete for QuoteRequest")
  ...
  def ap_receive_activity(_subject, activity, _object) do
    info(activity, "Unhandled quote activity")
