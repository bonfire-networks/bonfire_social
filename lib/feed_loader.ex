defmodule Bonfire.Social.FeedLoader do
  ...
  def feed(%FeedFilters{} = filters, opts) when is_list(opts) do
    with preset <-
           opts[:feed_preset] ||
             ...
             |> debug("using preset from opts or looked up from filters"),
         ...
         {filters, opts} <-
           ...
           |> debug("prepared filters and opts") do
  ...
  #   debug("Activity ID: #{activity.id} | Timestamp: #{ts}")
  # end)
  ...
  def feed_contains?(feed, objects_or_filters, opts)
    ...
    if Keyword.keyword?(objects_or_filters) and is_atom(feed) do
      case feed_contains_many(feed, objects_or_filters, opts) do
        ...
        items ->
          items
          |> debug("feed contained matching objects")
      end
    else
  ...
  def return_feed_contains_match(fi, fun) when is_function(fun, 1) do
    fun.(fi)
    # |> debug("returning match using custom function")
  end
