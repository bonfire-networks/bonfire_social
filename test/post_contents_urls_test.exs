defmodule Bonfire.Social.PostContentsURLsTest do
  ...
  test "editor-escaped YouTube URLs are extracted for preview fetching" do
    ...
    input =
      ~S(https://www.youtube.com/watch?v=17MBllYf6OY&list=RD17MBllYf6OY\&start\_radio=1 test)

    prepared =
      PostContents.parse_and_prepare_contents(%{html_body: input}, me, output_format: :markdown)

    assert prepared.urls == [
             "https://www.youtube.com/watch?v=17MBllYf6OY&list=RD17MBllYf6OY&start_radio=1"
           ]
  end
  ...
  test "different video IDs remain distinct after normalization" do
    ...
    urls =
      for video <- ["17MBllYf6OY", "x8SJteSVLnk"],
          do: "https://www.youtube.com/watch?v=#{video}&start_radio=1"
  ...
  test "code and ordinary Markdown escapes are preserved" do
    ...
    assert Text.normalise_markdown_urls("`#{url}` #{url}") ==
