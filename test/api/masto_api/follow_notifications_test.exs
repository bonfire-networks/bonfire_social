defmodule Bonfire.Social.API.FollowNotificationsTest do
  ...
  test "publishing ordinary posts and polls does not notify followers", c do
    ...
    refute Enum.any?(
             ...
             &(&1["status"] && &1["status"]["id"] in [ordinary["id"], direct["id"], poll["id"]])
           )
  end
  ...
  test "explicit mentions still notify through the native delivery path", c do
    ...
    status =
      c.author_conn
      |> post("/api/v1/statuses", %{"status" => "@#{username} #{Faker.Lorem.sentence()}"})
      |> json_response(200)

    assert Enum.any?(
             notifications(c.conn),
             &(&1["type"] == "mention" and &1["status"]["id"] == status["id"])
           )
