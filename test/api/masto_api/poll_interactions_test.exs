defmodule Bonfire.Poll.API.MastoPollTest do
  ...
  setup %{conn: conn} do
    ...
    {:ok, author: author, reader: reader,
  ...
  for multiple <- [false, true] do
    test "creates and reads a poll with multiple=#{multiple}", context do
      status = create_poll(context.author_conn, %{"multiple" => unquote(multiple)}) |> json_response(200)
  ...
  test "vote totals and own choices persist in poll and status reads", context do
    ...
    voted = context.reader_conn |> post("/api/v1/polls/#{question.id}/votes", %{"choices" => [0]}) |> json_response(200)
  ...
  for choices <- [[-1], ["0junk"], [0, "0junk"], [0, 99], [], [0, 1]] do
    test "rejects invalid single-choice ballot #{inspect(choices)} without saving votes", context do
      question = native_poll(context.author)
      context.reader_conn |> post("/api/v1/polls/#{question.id}/votes", %{"choices" => unquote(choices)}) |> response(422)
  ...
  test "rejects a second ballot", context do
    question = native_poll(context.author)
    context.reader_conn |> post("/api/v1/polls/#{question.id}/votes", %{"choices" => [0]}) |> json_response(200)
    context.reader_conn |> post("/api/v1/polls/#{question.id}/votes", %{"choices" => [1]}) |> response(422)
  end

  test "rejects expired polls", context do
    question = native_poll(context.author, -60)
    context.reader_conn |> post("/api/v1/polls/#{question.id}/votes", %{"choices" => [0]}) |> response(422)
  end

  for params <- [%{"options" => ["Only"]}, %{"expires_in" => 0}, %{"options" => ["", "Coffee"]}, %{"options" => [nil, "Coffee"]}] do
  ...
  test "multiple choice counts choices and unique voters separately", context do
    ...
    poll = context.reader_conn |> post("/api/v1/polls/#{status["poll"]["id"]}/votes", %{"choices" => [0, 1]}) |> json_response(200)
  ...
  test "hidden totals stay hidden to a voter before expiry", context do
    ...
    poll = context.reader_conn |> post("/api/v1/polls/#{status["poll"]["id"]}/votes", %{"choices" => [0]}) |> json_response(200)
    assert Enum.map(poll["options"], & &1["votes_count"]) == [nil, nil]
  end

  test "poll-only status is accepted", context do
    status = context.author_conn |> post("/api/v1/statuses", %{"visibility" => "public", "poll" => %{"options" => ["Tea", "Coffee"], "expires_in" => 60}}) |> json_response(200)
    assert status["poll"]["id"] == status["id"]
  end

  test "poll content warnings and reply relationships survive reads", context do
    parent = context.author_conn |> post("/api/v1/statuses", %{"status" => "Parent", "visibility" => "public"}) |> json_response(200)
    status = context.author_conn |> post("/api/v1/statuses", %{"status" => "Poll reply", "spoiler_text" => "Poll warning", "visibility" => "public", "in_reply_to_id" => parent["id"], "poll" => %{"options" => ["Tea", "Coffee"], "expires_in" => 3600}}) |> json_response(200)
    read = context.reader_conn |> get("/api/v1/statuses/#{status["id"]}") |> json_response(200)
  ...
  test "private poll is inaccessible to a non-follower", context do
    status = context.author_conn |> post("/api/v1/statuses", %{"status" => "Private poll", "visibility" => "private", "poll" => %{"options" => ["Tea", "Coffee"], "expires_in" => 3600}}) |> json_response(200)
  ...
  test "plain reply includes its parent in the create response and subsequent read", context do
    parent = context.author_conn |> post("/api/v1/statuses", %{"status" => "Parent", "visibility" => "public"}) |> json_response(200)
    reply = context.reader_conn |> post("/api/v1/statuses", %{"status" => "Reply", "visibility" => "public", "in_reply_to_id" => parent["id"]}) |> json_response(200)
  ...
  test "like and boost counts reflect both directions through GraphQL reads", context do
    ...
    for {action, field, count_field, expected, count} <- [
      ...
      response = context.reader_conn |> post("/api/v1/statuses/#{status["id"]}/#{action}", %{}) |> json_response(200)
  ...
  test "bookmark response preserves existing likes on a poll", context do
    status = create_poll(context.author_conn, %{}) |> json_response(200)
    context.reader_conn |> post("/api/v1/statuses/#{status["id"]}/favourite", %{}) |> json_response(200)
    response = context.reader_conn |> post("/api/v1/statuses/#{status["id"]}/bookmark", %{}) |> json_response(200)
  ...
  defp create_poll(conn, overrides) do
    post(conn, "/api/v1/statuses", %{"status" => "Poll regression", "visibility" => "public", "poll" => Map.merge(%{"options" => ["Tea", "Coffee"], "expires_in" => 3600}, overrides)})
  end

  defp native_poll(author, seconds \\ 3600) do
    now = DateTime.utc_now()
    {:ok, question} = Questions.create(current_user: author, boundary: "public", question_attrs: %{
      post_content: %{html_body: "Native poll regression"}, voting_format: "single",
