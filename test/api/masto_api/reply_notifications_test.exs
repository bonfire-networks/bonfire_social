defmodule Bonfire.Social.API.ReplyNotificationsTest do
  ...
  setup %{conn: conn} do
    ...
    {:ok, author: author, replier: replier,
  ...
  for poll? <- [false, true] do
    test "reply to poll=#{poll?} notifies its author without an explicit mention", context do
      ...
      params = if unquote(poll?), do: Map.put(params, "poll", %{"options" => ["Tea", "Coffee"], "expires_in" => 3600}), else: params
      parent = context.author_conn |> post("/api/v1/statuses", params) |> json_response(200)
      reply = context.replier_conn |> post("/api/v1/statuses", %{
        "status" => Faker.Lorem.sentence(), "in_reply_to_id" => parent["id"], "visibility" => "public"}) |> json_response(200)
      ...
      self_notifications = context.replier_conn |> get("/api/v1/notifications") |> json_response(200)
  ...
  test "replying to oneself does not notify oneself", context do
    parent = context.author_conn |> post("/api/v1/statuses", %{"status" => Faker.Lorem.sentence()}) |> json_response(200)
    reply = context.author_conn |> post("/api/v1/statuses", %{"status" => Faker.Lorem.sentence(), "in_reply_to_id" => parent["id"]}) |> json_response(200)
    notifications = context.author_conn |> get("/api/v1/notifications") |> json_response(200)
