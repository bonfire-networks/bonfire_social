defmodule Bonfire.Poll.API.CompletionTest do
  ...
  test "completion is scheduled, never early, and delivered once to author and voter", context do
    ...
    jobs =
      Oban.Job
      |> where([j], j.worker == "Bonfire.Poll.CompletionWorker")
    ...
    for conn <- [context.conn, context.voter_conn],
        do: assert(completions(conn, poll["id"]) == [])
    ...
    for conn <- [context.conn, context.voter_conn] do
      ...
      assert Map.take(entry, ["created_at", "content", "account"]) ==
               Map.take(canonical, ["created_at", "content", "account"])
    end
  end
  ...
  test "a voter who loses private-poll access is not notified at completion", context do
    ...
    refute Bonfire.Common.Repo.exists?(
             from p in Bonfire.Data.Social.FeedPublish,
               where:
                 p.id == ^completed.completion_activity_id and p.feed_id == ^notifications_feed
           )
  end

  defp completions(conn, id) do
    conn
    |> get("/api/v1/notifications?types[]=poll")
    |> json_response(200)
