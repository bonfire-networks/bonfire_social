defmodule Bonfire.Social.API.GroupedNotificationsTest do
  ...
  test "groups two favourites, preserves canonical references, and keeps v1 flat", %{
    conn: conn,
    user: user
  } do
    {:ok, post} =
    ...
    assert MapSet.new(Enum.map(result["accounts"], & &1["id"])) ==
             MapSet.new(group["sample_account_ids"])
    ...
    assert length(conn |> get("/api/v1/notifications?types[]=favourite") |> json_response(200)) ==
             2
  end

  test "paginates activities before grouping without losing or repeating notifications", %{
    conn: conn,
    user: user
  } do
    ...
    {groups, next} =
      Enum.reduce(1..6, {[], "/api/v2/notifications?limit=1&types[]=favourite"}, fn _,
                                                                                    {groups, url} ->
        refute is_nil(url)
        ...
        next =
          case get_resp_header(page_conn, "link") do
            [links] ->
              case Regex.run(~r/<([^>]+)>; rel="next"/, links) do
                [_, url] ->
                  uri = URI.parse(url)
                  uri.path <> "?" <> uri.query
    ...
    ungrouped =
      conn
      |> get("/api/v2/notifications?grouped_types[]=follow&types[]=favourite")
      |> json_response(200)

    assert length(ungrouped["notification_groups"]) == 6

    assert Enum.all?(
             ungrouped["notification_groups"],
             &(String.starts_with?(&1["group_key"], "ungrouped-") and
                 &1["notifications_count"] == 1)
           )

    filtered =
      conn |> get("/api/v2/notifications?account_id=#{hd(actors).id}") |> json_response(200)

    assert length(filtered["notification_groups"]) == 2

    assert Enum.all?(
             filtered["notification_groups"],
             &(&1["sample_account_ids"] == [hd(actors).id])
           )
  end

  test "follows stay separate and group details are explicitly unsupported", %{
    conn: conn,
    user: user
  } do
    actors = for _ <- 1..2, do: Bonfire.Me.Fake.fake_user!()
  ...
  test "native boost groups keep page-local actors and continue across page boundaries", %{
    conn: conn,
    user: user
  } do
    {:ok, post} =
    ...
    assert MapSet.new(group["sample_account_ids"] ++ remaining["sample_account_ids"]) ==
             MapSet.new(Enum.map(actors, & &1.id))
    ...
    assert (outsider
            |> get("/api/v2/notifications?types[]=reblog")
            |> json_response(200))["notification_groups"] == []
  end
  ...
  test "since and min cursors return only newer groups", %{conn: conn, user: user} do
    ...
    result =
      conn
      |> get("/api/v2/notifications?since_id=#{middle}&types[]=favourite")
      |> json_response(200)
    ...
    result =
      conn
      |> get("/api/v2/notifications?min_id=#{oldest}&limit=1&types[]=favourite")
      |> json_response(200)
